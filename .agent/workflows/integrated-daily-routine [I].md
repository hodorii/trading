---
description: 팩트 중심 분석 및 전략적 근거 집대성 루틴
---

# 🔄 integrated-daily-routine [I]

## 🎯 목표
[Phase 0-3]을 통한 정밀 실측 및 최종 변론서 작성.

## 🚨 준수: [PROTOCOLS.md] 전 항목 (GP-ETO, GP-SPEC, GP-EBR, GP-VFY, GP-REV)

---

## 🛠️ Pipeline (9-Step)

### [Phase 0: Review]
- **0. Review**: [GP-REV] 수행. 어제 결정 대비 오늘 성과 실측.

### [Phase 1: Intelligence]
- **1. [매크로]**: NQ=F, ES=F, VIX, 환율 민감도 분석.
- **2. [이벤트]**: 뉴스/공시 팩트체크 및 국내 전이(Spillover) 경로 실측.
- **3. [시장]**: US-Linked 섹터 주도권 및 대장주 거래대금 비중 분석.

### [Phase 2: Deep Dive]
- **4. [가치]**: 3개년 PER/PBR 밴드 대조 및 적정가 산식 포함.
- **5. [수급]**: 창구별 유입 시각 포렌식. '검은머리 외인' 판별 필수.
- **6. [리스크]**: [GP-VFY] 기반 데이터 괴리 및 De-coupling 진단.

### [Phase 3: Synthesis]
- **7. [합성] (Tactical Synthesis)**: 0-6단계 근거 집대성. **함축 금지**. 모순점(예: 가치 vs 수급) 적시.
- **8. [요약]**: 세션 인덱스 및 Why-First 3줄 압축.

---
**ID**: `IDR-LEAN`
