# Antigravity: Multi-Agent Trading Intelligence System

> **"지수와 종목을 잇는 입체적 시각, 데이터 오염 없는 순수한 의사결정"**

Antigravity는 MoE(Mixture of Experts) 아키텍처를 기반으로 한 차세대 에이전틱 트레이딩 인텔리전스 시스템입니다. 한국 주식 시장의 특수성(NXT 연계, 수급 포렌식)과 글로벌 지수(나스닥, S&P 500)의 상관관계를 실시간으로 분석하여 기관급 리서치와 실전 전략을 생성합니다.

---

## 🏗️ Core Architecture: 3-Tier Multi-Agent System

Antigravity는 단순히 데이터만 나열하는 것이 아니라, **운영 - 생산 - 판단**의 3단계 계층 구조를 통해 가장 냉철한 투자 결정을 도출합니다.

### 🟢 Tier 1: 운영 (TTO - Timeline Trading Ops)
-   시점별(장전/장중/장후) 최적 분석 모드를 결정하는 **마스터 스케줄러**.

### 🔵 Tier 2: 생산 (IDR & [T] Skills)
-   **IDR**: 6대 전문가(매크로, 이벤트, 섹터, 가치, 수급, 리스크)의 객관적 **팩트 리포트** 생산.
-   **Tactical [T]**: 데이트레이딩, 스윙 등 스타일별 **실전 전술 리포트** 생산.

### 🔴 Tier 3: 판단 (MAD - Multi-Agent Decision)
-   IDR과 Tactical의 산출물을 집대성하여 에이전트 간 난상토론을 거쳐 **'실전 쉬운 가이드'**를 생산하는 **최종 종결자**.

---

## ✨ Key Features

### 1. **통합 자율 연계 분석 (Autonomous Decision Chain)**
-   사용자가 최종 결정(`MAD`)만 호출해도, 데이터 부재 시 시스템이 스스로 판단하여 팩트 생산(`IDR`)을 먼저 수행하는 지능형 의존성(Dependency)을 갖추고 있습니다.

### 2. **실전 쉬운 가이드 (Practical Easy Guide)**
-   복잡한 데이터 분석 결과를 비전공자도 즉시 실행할 수 있는 '매수/매도 로드맵'으로 요약하여 제공합니다.

### 3. **실전 미세 전술 (Tactical Precision)**
'라운드 피겨 매도 원칙', '08:30 NXT 선제 테마 포착' 등 실전 복기를 통해 검증된 미세 전술이 워크플로우에 내장되어 있습니다.

### 4. **글로벌 데이터 체인**
국내 데이터의 편향성을 극복하기 위해 Yahoo Finance, DART, 미국 선물 지수를 실시간 동기화하여 분석합니다.

---

## 🛠️ Technology Stack & Tools

-   **Intelligence**: Google DeepMind Agentic AI Logic
-   **Data Sources**: KIS API (Remote), Naver Finance API, Yahoo Finance API
-   **Framework**: Agentic Workflow Design (MoE Pipeline)
-   **Language Policy**: **Korean-First** (모든 분석 및 보고서 한국어 준수)

---

## 📂 Project Structure

-   `.agent/`: 전문가용 스킬(Skills) 및 워크플로우(Workflows) 정의
-   `.gemini/`: 에이전트의 페르소나 및 절대 원칙(`GEMINI.md`)
-   `reports/`: 분석된 리포트 자산 (Git 추적 제외)
-   `CHANGELOG.md`: 프로젝트 진화 및 변경 이력

---

## 🚀 Quick Start

1.  **에이전트 로딩**: `antigravity`를 기동하고 환경 변수를 확인합니다.
2.  **분석 및 결정**: `/multi-agent-decision 종목명`을 호출합니다. (IDR 팩트 부재 시 자동으로 선행 수행됨)
3.  **리포트 확인**: `reports/YYYY-MM-DD/` 폴더에서 생성된 팩트 리포트 6종과 MAD의 **'최종통합전략'**, **'실전쉬운가이드'** 패키지를 검토합니다.

---

## 📜 Principles & Philosophy

-   **Fact-Based**: 모든 가설은 데이터로 증명합니다.
-   **SRP (Single Responsibility)**: 각 전문가는 본인의 영역에만 집중합니다.
-   **Integrity**: Git 저장소를 통해 모든 지식과 원칙을 버전 관리합니다.

---
*Developed by antigravity (Agentic AI Intelligence)*
